...
public class Utils {
    ...
    public static void info(String text) {
        Bukkit.getConsoleSender().sendMessage("[BedWars-QuickBuyManager] " + color(text));
    }

    public static void sendActionBar1058(Player player, String message) {
        JavaPlugin.getPlugin(QuickBuyManager.class);
        QuickBuyManager.bw1058Api.getVersionSupport().playAction(player, color(message));
    }

    public static void sendActionBar2023(Player player, String message) {
        JavaPlugin.getPlugin(QuickBuyManager.class);
        QuickBuyManager.bw2023Api.getVersionSupport().playAction(player, color(message));
    }
